...
def train_and_validate(args):
    ...
    model = DialFactClassification(args.pretrain_model_dir,num_labels=3,
                                   is_IGM=args.is_IGM, is_LabelEmb=args.is_LabelEmb, iter_num=args.iter_num, LabelEmb_dim=args.LabelEmb_dim)
    model.to(device)
    ...
    for epoch in range(args.max_epochs):
        ...
        score = meters['acc']
        logging.info(f"acc = {meters['acc']},  macro_f1 = {meters['macro_f1']}, macro_recall = {meters['macro_recall']}, macro_precision = {meters['macro_precision']}")
        ...
        model_path = os.path.join(args.save_model_path, f'epoch{epoch + 1}_acc_{score}.bin')
        state_dict = model.state_dict()
        torch.save(state_dict, model_path)
