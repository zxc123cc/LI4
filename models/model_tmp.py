...
from cl_loss_func import DialFactDualLoss
...
class DialFactClassification(nn.Module):
    ...
    def __init__(self, model_dir,num_labels=3):
        super().__init__()
        self.base_model = RobertaForSequenceClassification.from_pretrained(model_dir,num_labels)
...
class RobertaForSequenceClassification(RobertaPreTrainedModel):
    ...
    def __init__(self, config, num_labels):
        ...
        self.classifier = RobertaClassificationHead(config,vec_num=4)
        ...
        self.criterion = DialFactDualLoss(alpha=0.5, temp=0.1, neutral_cl_alpha=0.2)
    ...
    def forward(
            ...
            claim_ids=None,
            claim_alone_mask=None,
            evidence_ids=None,
            evidence_alone_mask=None,
            question_ids=None,
            question_alone_mask=None
    ):
        r"""
        ...
        # output_tmp = self.classifier(None,x=cat_vector,label_feats=label_feats)
        # logits = output_tmp['predicts']
        ...
        if labels is not None:
            ...
            if self.config.problem_type == "regression":
                ...
                # loss = loss_fct(logits.view(-1), label_tmp.float().view(-1)) #将其看成二分类（bs）
                # loss = self.criterion(output_tmp, labels.view(-1))
...
class RobertaClassificationHead(nn.Module):
    ...
    def __init__(self, config, vec_num=1):
        super().__init__()
        self.config = config
...
#         self.embedding_dense = nn.Linear(config.hidden_size, self.embed_dim)
#         # self.embedding_out_proj = nn.Linear(config.hidden_size + self.embed_dim * config.num_labels, config.num_labels)
#         # self.embedding_out_proj = nn.Linear(config.hidden_size + self.embed_dim, 1)
#
#     def forward(self, features, x=None, label_feats = None, **kwargs):
#         if x is None:
#             x = features[:, 0, :]  # take <s> token (equiv. to [CLS])
...
#         if label_feats is not None:
#             label_feats = self.dropout(label_feats)
#             label_info = self.embedding_dense(label_feats)
#
#             # emb_x = torch.cat([x,label_info[:,0,:],label_info[:,1,:],label_info[:,2,:]],dim=1)
#             # emb_x = torch.tanh(emb_x)
#             # emb_x = self.dropout(emb_x)
#             # emb_x = self.embedding_out_proj(emb_x)
#             # return emb_x
#
#             # x = torch.cat([x.unsqueeze(1) for _ in range(3)],dim=1) # bs 3 1024
#             # emb_x = torch.cat([x,label_info],dim=-1) # bs 3 1024+256
#             # emb_x = torch.tanh(emb_x)
#             # emb_x = self.dropout(emb_x)
#             # output = self.embedding_out_proj(emb_x)
#             # logits = output.squeeze(-1) # bs,3
#             # return logits
#
#             predicts = torch.einsum('bd,bcd->bc', x, label_info)
#             # return predicts
#             outputs = {
#                 'predicts': predicts,
#                 'cls_feats': x,
#                 'label_feats': label_info
#             }
#             return outputs
#
#         x = torch.tanh(x)
